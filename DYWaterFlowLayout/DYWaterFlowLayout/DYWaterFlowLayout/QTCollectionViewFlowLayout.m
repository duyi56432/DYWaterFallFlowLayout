...
//
//  QTCollectionViewFlowLayout.m
//  QTAssistant
//
//  Created by duyi on 2021/8/12.
//

#import "QTCollectionViewFlowLayout.h"

@implementation QTFlowLayoutModel
...
@interface QTCollectionViewFlowLayout()
...
@implementation QTCollectionViewFlowLayout
...
- (NSMutableArray *)sectionArray {
    if (!_sectionArray) {
        ...
        for (int i = 0; i < [self numberOfSections]; i++) {
            QTFlowLayoutModel *model = [[QTFlowLayoutModel alloc] init];
...
- (void)updateAttributes:(NSInteger)section {
    ...
    QTFlowLayoutModel *model = self.sectionArray[section];
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect {
    ...
    for (QTFlowLayoutModel *model in self.sectionArray) {
...
- (void)setCellFrameVertical:(NSArray *)layoutAttributes section:(NSInteger)section {
    ...
    for (int index = 0; index < layoutAttributes.count; index ++) {
        ...
        if (index < model.numOfRow && model.rowHeightArray.count < model.numOfRow) {
            ...
            CGSize size = [model.rowHeightArray[minNum] CGSizeValue];
            nowFrame.origin.x = size.width;
            nowFrame.origin.y = yMinPoint + model.lineSpace;
            currentAttr.frame = nowFrame;
            
            model.rowHeightArray[minNum] = [NSValue valueWithCGSize:CGSizeMake(currentAttr.frame.origin.x, currentAttr.frame.size.height + currentAttr.frame.origin.y)];
        }
